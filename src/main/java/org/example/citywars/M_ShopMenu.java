...
public class M_ShopMenu extends Menu{
    ...
    public Menu myMethods(){
        ...
        do {
            ...
            if (input.matches("^show new cards$")){
            ...
            }else if (input.matches("^add card -name (?<name>[a-zA-Z]+) -prc (?<price>[0-9]+) -dur (?<duration>[0-9]+) -type (?<type>(Regular|Spell)+) -pow (?<power>[0-9]+) -dmg (?<damage>[0-9]+) -upl (?<upgradeLevel>[0-9]+) -upc (?<upgradeCost>[0-9]+) -chr (?<gameCharacter>[a-zA-Z]+)$")) {
                matcher = Pattern.compile("^add card -name (?<name>[a-zA-Z]+) -prc (?<price>[0-9]+) -dur (?<duration>[0-9]+) -type (?<type>(Regular|Spell)+) -pow (?<power>[0-9]+) -dmg (?<damage>[0-9]+) -upl (?<upgradeLevel>[0-9]+) -upc (?<upgradeCost>[0-9]+) -chr (?<gameCharacter>[a-zA-Z]+)$").matcher(input);
                matcher.find();
                ...
                try {
                    res = CardController.createCard(loggedInUser, matcher.group("name"),Integer.parseInt(matcher.group("price")),Integer.parseInt(matcher.group("duration")),matcher.group("type"),Integer.parseInt(matcher.group("power")),Integer.parseInt(matcher.group("damage")),Integer.parseInt(matcher.group("upgradeLevel")),Integer.parseInt(matcher.group("upgradeCost")), desc,matcher.group("gameCharacter") );
                    System.out.println(res.message);
                    if( res.exception != null ){
                        System.out.println(res.exception.getMessage());
                    }
                ...
                fetch();
            } else if (input.matches("^edit card -num (?<number>[0-9]+) -name (?<name>[a-zA-Z]+) -prc (?<price>[0-9]+) -dur (?<duration>[0-9]+) -pow (?<power>[0-9]+) -dmg (?<damage>[0-9]+) -upl (?<upgradeLevel>[0-9]+) -upc (?<upgradeCost>[0-9]+) -chr (?<gameCharacter>[a-zA-Z]+)$")) {
                matcher = Pattern.compile("^edit card -num (?<number>[0-9]+) -name (?<name>[a-zA-Z]+) -prc (?<price>[0-9]+) -dur (?<duration>[0-9]+) -pow (?<power>[0-9]+) -dmg (?<damage>[0-9]+) -upl (?<upgradeLevel>[0-9]+) -upc (?<upgradeCost>[0-9]+) -chr (?<gameCharacter>[a-zA-Z]+)$").matcher(input);
                matcher.find();
                System.out.println("please enter one line as a description for card: ");
                String desc = consoleScanner.nextLine();
                if( desc == null || desc.isBlank() ){
                ...
                try {
                    res = CardController.editCard(loggedInUser,allCards.get(Integer.parseInt(matcher.group("number"))).getID(),matcher.group("name"),Integer.parseInt(matcher.group("price")),Integer.parseInt(matcher.group("duration")),Integer.parseInt(matcher.group("power")),Integer.parseInt(matcher.group("damage")),Integer.parseInt(matcher.group("upgradeLevel")),Integer.parseInt(matcher.group("upgradeCost")),desc,matcher.group("gameCharacter") );
                    System.out.println(res.message);
                    if( res.exception != null ){
                        System.out.println(res.exception.getMessage());
                    }
